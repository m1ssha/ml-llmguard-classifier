...
from classifier.src.dataset.validator import build_adversarial_holdout_splits, build_distribution_report, validate_samples
...
from classifier.src.models.encoder import (
    save_encoder_artifacts,
    save_inference_artifact_bundle,
    train_encoder_pipeline,
    train_stronger_encoder_pipeline,
)
...
def train_baseline_pipeline(samples: list[dict], config: ReproConfig) -> dict:
    ...
    validate = validate_samples(samples, strict_adversarial=True)
    ...
    _save_metrics(
        metrics_path,
        {
            ...
            "distribution": build_distribution_report(validate),
            "adversarial_holdouts": {k: len(v) for k, v in holdout_splits.items()},
        },
    )
...
def train_stronger_encoder_training_pipeline(samples: list[dict], config: ReproConfig) -> dict:
    set_seed(config.seed)
    validate = validate_samples(samples, strict_adversarial=True)
    check_shortcut_patterns(samples)
    holdout_splits = build_adversarial_holdout_splits(samples)
    ...
    model = train_stronger_encoder_pipeline(x_train, y_train, random_state=config.seed)
    y_pred = model.predict(x_test).tolist()
    multiclass = compute_multiclass_metrics(y_test, y_pred, labels=sorted(set(labels)))
    bundle = MetricsBundle(binary={"accuracy": 1, "precision": 1, "recall": 1, "f1": 1, "roc_auc": 1}, multiclass=multiclass)
    validate_metrics_completeness(bundle)
    ...
    exp_dir = create_experiment_dir(config)
    save_config_snapshot(config, exp_dir, extra={"pipeline": "stronger_encoder"})
    thresholds = {
    ...
    artifacts = save_inference_artifact_bundle(
        ...
        metadata={"seed": config.seed, "train_size": len(x_train), "test_size": len(x_test), "candidate": "stronger_encoder"},
    )
    metrics_path = exp_dir / "stronger_encoder_metrics.json"
    _save_metrics(
        metrics_path,
        {
            "multiclass": multiclass,
            "distribution": build_distribution_report(validate),
            "adversarial_holdouts": {k: len(v) for k, v in holdout_splits.items()},
            "deployment_profiles": thresholds,
        },
    )
    ...
    return {
        ...
        "tokenizer_path": str(artifacts.tokenizer_path),
        "metadata_path": str(artifacts.metadata_path),
        "label_mapping_path": str(artifacts.label_mapping_path),
